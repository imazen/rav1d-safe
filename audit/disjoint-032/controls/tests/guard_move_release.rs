...
// The panic hook is process-global. Keep the two test functions from replacing
// each other's hook; worker threads within each test remain concurrent.
                ...
                match result {
                    ...
                    Err(error) => {
                        let text = error
                            ...
                            .unwrap_or("");
                        if !text.contains("overlapping DisjointMut") {
                            panic::resume_unwind(error);
                        }
                        refused.fetch_add(1, Ordering::Relaxed);
                        // The 0.3 tracker also locks during retirement. Let the
                        // holder retire instead of starving it with failed
                        // admissions. Preserve all rounds and liveness floors.
