#!/usr/bin/env bash
# Interleaved, order-rotated A/B sweep. All arms for a cell run back to back
# within a round; the arm order rotates every round so drift cannot land on one
# arm. Refuses to start (and discards the round's cell) while the box is busy.
set -u
OUT=${1:?out.tsv}; ROUNDS=${2:-3}; REPS=${3:-3}; ITERS=${4:-4}
BIN=$HOME/tmp/rav1d-p1/bin
VEC=$HOME/tmp/rav1d-perf/vec
ARMS=(base tt ttu ttg ttng)
CELLS=("v4k_8tile:1" "v4k_8tile:2" "v4k_8tile:4" "v4k_8tile:8" "v4k_8tile_10b:8" "v4k_8tile_10b:4")
