...
for f in "" __bps_quarter __bps_half __bps_1 __bps_4 __bps_8 __bps_blocks __msb_5 __msb_5,__bps_blocks; do
  if [ -z "$f" ]; then
...
VECS=${MDP_VECS:-"C1024x576_420_8b__t8 C3840x256_420_8b__t8 C256x2048_420_8b__t8"} \
  nice -n 19 bash scripts/perf/multi_decoder_pressure.sh \
  "${MDP_BIN_ROWS:-$HOME/tmp/shardsize/bin/bench_bpsblocks}" 12 3 600 \
