#!/usr/bin/env bash
# Independent gap-to-dav1d sweep, with per-arm ENV so runtime-switched arms
# (`head`/`headoff`, one binary, `RAV1D_OWNED_RECON`) interleave in one sweep.
# Forked from verify_gap.sh; everything below the arm dispatch is identical.
#
...
#
# Usage: verify_gap_arms.sh <out.tsv> [rounds]
...
time_one() {
  ...
  case "$arm" in
    ...
    # `head` and `headoff` are the SAME BINARY: #482's conversion is armed by a
    # RUNTIME switch precisely so an inter-arm delta cannot be a codegen
    # artefact. `headoff` is every frame the conversion declines, i.e. every
    # inter frame today, and it is the arm that decides whether the seam taxes
    # the tracked path.
    head)      RAV1D_OWNED_RECON=1 "$BIN/bench_head" "$AVIF/$vec.avif" "$t" "$n" 1 w >/dev/null 2>&1 ;;
    headoff)   RAV1D_OWNED_RECON=0 "$BIN/bench_head" "$AVIF/$vec.avif" "$t" "$n" 1 w >/dev/null 2>&1 ;;
