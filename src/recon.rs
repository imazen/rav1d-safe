...
fn decode_coefs<BD: BitDepth>(
    ...
    t_cf: &mut Cf,
    // `a` and `l` are read-only context bytes consumed by `get_skip_ctx`
    // (and nothing else in this function). Taking them as `&[u8]` lets callers
    // use a cheaper immutable DisjointMut guard (`add_immut` only scans
    // mutable slots), and — more importantly — lets the per-block CaseSet
    // memset that follows acquire its own mutable guard without needing to
    // batch lifetimes through this large function.
    a: &[u8],
    l: &[u8],
    tx: TxfmSize,
    bs: BlockSize,
    ...
    *txtp = match &b.ii {
    ...
    impl<'a, BD: BitDepth> Cf<'a, BD> {
        ...
        #[cfg_attr(any(debug_assertions, feature = "probe-sites", feature = "probe-usage"), track_caller)]
