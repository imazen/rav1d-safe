...
> **Landed 2026-08-12 from PR #489, records only.** Written 2026-08-10 against
> `675a7ff`; five PRs merged in between (#500-#506) and three of them bear on
> what is below. Corrections are dated in place, never by deletion:
>
> * **The "#469 rectangle → BLOCKED" verdict is about the HULL, and #505 has
>   since built an exact rectangle that this verdict does NOT condemn.** See the
...
> **SCOPE CORRECTION, 2026-08-11 (PR #505, merged `e351b1a`) — read this before
> quoting "the rectangle is BLOCKED".** What the ROWBAND counterfactual tests is
> the **hull**: every byte of every picture row the reservation spans, gaps
> included. That is what a *hull* record hands back, and it is BLOCKED, 18,196
> times, exactly as stated. It is **not** what an exact strided rectangle
> registers. `eval_rect` — the probe added to the front half of this file, "The
> strided-rectangle counterfactual" — measures the other shape and reports
> **`rect_ovl = 0` at every site**: every hull collision is a *gap* collision,
> and the exact rectangle never collided once.
>
> #505 then built that exact record (`index_rect_as`, `DisjointImmutRectGuard`),
> which also closes the second defect this section says the map cannot see: the
> guard has **no `Deref`** and derives each row from the buffer's own pointer, so
> the record and the reference are the same object (Miri-clean under both
> aliasing models). It is default-off, and its measured result is a **t=8 win** —
> −1.0% to −1.8% wall on 5 of 6 multi-tile cells — not a soundness failure.
> `docs/RECT_RECORDS.md`, `docs/RECT_SHIP.md`.
>
> So: **hull → BLOCKED (this section, still correct). Exact rectangle → NOT
> blocked, built, and measured.** Do not carry this row forward as a verdict on
...
> **THE RANKING KEY IS SUPERSEDED, 2026-08-11 (PR #505, `docs/RECT_RECORDS.md`;
> `docs/AGENT_BRIEF.md` §6).** These rows are ordered by **registrations/frame**,
> and registrations/frame is now measured to be a poor proxy for cost. The
> tracker's price tracks **distinct shard LINES a worker visits**, not records
> filed: a repeat touch of a line the core already owns is ~7.7x cheaper than a
> first touch. Measured at the decoder's largest site — `LfBlock::fill` files
> **8.98 records per 2.09 distinct shard lines**, is **31.7%** of the whole
> registration population, and is only **3.9-4.4%** of the tracker's CPU
> (`RAV1D_LF_DOUBLE`, a doubling arm in ONE binary: 2.42-2.71 ns per `fill`
> registration against a whole-tracker average of 19.71 ns/registration at
> `c256x2048` t=8). **The average was being read as a marginal price, and it is
> not one.** Consequence for this list: a count cut at the *densest* site is the
> least valuable one available, so **re-price every row below against
> `row_shards_mean` in `benchmarks/bounds_verdicts_2026-08-10.tsv` before acting
> on its rank.** The verdicts themselves (which site can be widened, and by how
> much) are unaffected — this corrects the ORDER, not the soundness column.
>
> A second, related refutation worth having here: the cost model
> `rows_mean / row_shards_mean` at the `fill` site does **not** order the cells
> either — `c256x2048` has the highest ratio (4.30 vs the winners' 3.85-3.88) and
> is the one null (`docs/RECT_SHIP.md`). `pct_row_wide == 0` separates the two
> weak cells but is necessary, not sufficient. **No published model currently
> orders the t=8 win.**
