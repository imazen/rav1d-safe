...
> **Landed 2026-08-12 from PR #490, records-only.** Two things changed between the
> measurement and the landing, and both are flagged in place below rather than
> edited out:
>
> 1. **§5's code change is NOT in this commit.** `src/safe_simd/cdef_arm.rs`'s
>    `copy_row_u16` is a live (non-gated) change with a measured-null verdict, so
>    it was split out of this records landing for its own decision — see §5's
>    banner. Its measurement records are kept here, because they are the evidence
>    for the *no* as much as for the mechanism.
> 2. **§8's ranked item 2 (`ctx.rs:99`'s `t.l` half) has since been DONE.** See
>    §8's note.
>
> Everything else — §1's census, §2's R² = 0.001, §3's screen-content ratios,
> §4's profile split, §6's load tag, §7's gates — is unchanged and is still the
> current position on `main`.
...
> **NOT LANDED. 2026-08-12: this section documents a change that is NOT in the
> tree.** `copy_row_u16` / `copy_n::<N>` in `src/safe_simd/cdef_arm.rs` were
> measured on branch `perf/size-hump-itx16` at `be289f4` and are not on `main`.
> They were split out of the records landing because the change is *live* — not
> gated behind a `__`-prefixed feature, not dead without one — and its own
> verdict is that it is not a measured win, which is a maintainer call rather
> than a records question. The A/B below, the memmove profile, the teeth-planted
> liveness proof and `benchmarks/cdef16_*_2026-08-10.*` are all kept, because
> they are the evidence for the **no** at least as much as for the mechanism.
> To evaluate or land the change itself, see PR #490's `be289f4`.
...
* the **8bpc control at 1024x576: +0.94%, 3 of 9** — on a code path the change
  ...
  **CONFIRMED 2026-08-11 by PR #506, which built the instrument this paragraph
  was inferring from a control cell.** `--features __pad_text` adds provably-dead
  `#[used]` text — 0 symbols resized, a planted `panic!` in the pad leaves the
  decode md5 unchanged — and costs **+1.1% to +1.6%** at t=1, while a
  byte-identical copy of the same binary reads **1.0006**. So this section's
  inference ("~1% is layout, not the change") was right, the floor generalises
  to **±1.5% on any t=1 arm**, and the tax scales with working set (+1.4% at 4K,
  +0.7% at 1024x576, 0 at 256x2048). `docs/RECT_SHIP.md`.
...
2. **`ctx.rs:99`'s `t.l` half** — 22% of every registration on this ladder at the
   ...
   > **DONE, 2026-08-10/11 — this item is closed.** The shape blocker was solved
   > and the item shipped across three merged PRs: **#492** (`perf/ctx-tl-split`,
   > the write side — 21-25% of registrations cut on screen content, −11.8% wall
   > at t=8), **#495** (`perf/ctx-tl-reads`, another ~11% of registrations, real
   > on screen text) and **#502** (`perf/ctx-read-split`, the LEFT neighbour's
   > reads leave the tracker — 10.5-13.9% of registrations, 0.9842 geomean).
   > `docs/CTX_TL_SPLIT.md`. Item 1 (profile-and-fix screen content) is also no
   > longer un-started: `text_q20` / `ui_q20` are standing cells in every sweep
   > from #502 onward. Item 3 stands as written.
