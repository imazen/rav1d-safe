...
### Fixed
- **aarch64 decode is bit-exact with x86 again** (issue #400): the aarch64 NEON
  inverse transforms (`itx_arm`) were not bit-exact with the scalar/spec
  reference — their NEON-vs-scalar unit tests pass only under a `MAX_DIFF = 15`
  tolerance, but AV1 decode requires exact output, so aarch64 produced wrong
  pixels (the same stream decoded to a different YUV MD5 on aarch64 vs x86 —
  e.g. kodim03, alpha_noispe — and the `differential_dav1d` fuzz target diverged
  from dav1d on arm64). The NEON itx dispatch now falls back to the proven
  generic scalar itx (verified byte-identical to x86 at every CPU level), the
  same correctness-first pattern as `loopfilter_arm`. The NEON kernels are
  preserved behind the never-set `rav1d_arm_itx_bitexact` cfg for re-enabling
  once each transform is verified bit-exact. New `tests/decode_md5_committed.rs`
  pins committed vectors to their x86 reference MD5 and runs on x86 **and**
  native arm64 in CI, so arch-specific decoder divergence fails the build instead
  of shipping wrong pixels. The dispatch mapping itself was correct (itx.rs uses
  a deliberately flipped row/col convention) — the bug was in the SIMD
