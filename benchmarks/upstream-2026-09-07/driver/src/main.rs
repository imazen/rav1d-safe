...
use rav1d::{Decoder, Picture as Frame, PixelLayout, PlanarImageComponent, Rav1dError, Settings};
...
fn frame_md5(frame: &Frame) -> String {
    ...
    let bytes = if frame.bit_depth() > 8 { 2 } else { 1 };
    let layout = frame.pixel_layout();
    for component in [PlanarImageComponent::Y, PlanarImageComponent::U, PlanarImageComponent::V] {
        let chroma = component != PlanarImageComponent::Y;
        if chroma && layout == PixelLayout::I400 { continue; }
        let sub_x = usize::from(chroma && matches!(layout, PixelLayout::I420 | PixelLayout::I422));
        let sub_y = usize::from(chroma && layout == PixelLayout::I420);
        let w = (frame.width() as usize).div_ceil(1 << sub_x) * bytes;
        let h = (frame.height() as usize).div_ceil(1 << sub_y);
        // Upstream's stride field is in bytes, despite its method documentation.
        let stride = frame.stride(component) as usize;
        assert!(stride >= w);
        let data = frame.plane(component);
        for row in 0..h { ctx.consume(&data[row * stride..row * stride + w]); }
    }
...
fn decoder(threads: u32) -> Decoder {
    ...
    s.set_frame_size_limit(120_000_000);
    s.set_strict_std_compliance(true);
    Decoder::with_settings(&s).expect("create decoder")
}

fn drain(dec: &mut Decoder, frame: &mut impl FnMut(Frame)) -> usize {
    let mut n = 0;
    loop {
        match dec.get_picture() {
            Ok(f) => { frame(f); n += 1; }
            Err(Rav1dError::TryAgain) => return n,
            Err(e) => panic!("drain picture: {e}"),
        }
    }
}
...
fn run(dec: &mut Decoder, packets: &[Vec<u8>], mut frame: impl FnMut(Frame)) -> usize {
    ...
    for packet in packets {
        let mut result = dec.send_data(black_box(packet).clone().into_boxed_slice(), None, None, None);
        let mut retries = 0;
        while result == Err(Rav1dError::TryAgain) {
            n += drain(dec, &mut frame);
            result = dec.send_pending_data();
            retries += 1;
            assert!(retries < 100_000, "pending data made no progress");
        }
        result.expect("decode packet");
        n += drain(dec, &mut frame);
    }
    n += drain(dec, &mut frame);
    dec.flush();
...
fn main() {
    ...
    println!(
        "CONFIG\tpackets={}\tframes={}\tthreads={}\tinstances={}\tpasses={}\treps={}\tframe_delay=1\tusage={}\twide={}\ttasks={}",
        input.len(),
        frames,
        threads,
        instances,
