...
fn main() {
    ...
    // Diagnostic process: count all visible decodes, with no counter resets
    // and no disabled kernel families. RESULT times are never speed evidence.
    assert!(rav1d_safe::src::ablate::ENABLED);
    let census = rav1d_safe::src::ablate::itx_shape_report();
    assert!(census.lines().count() > 1, "transform census was empty");
    println!("ITX_CENSUS_BEGIN");
    print!("{census}");
    println!("ITX_CENSUS_END");
}
