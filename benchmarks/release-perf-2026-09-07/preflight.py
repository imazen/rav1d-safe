...
records=[]
for arm in ['current-checked','current-unchecked','current-asm','first-checked','first-unchecked','example-control','upstream']:
 for name,path in inputs.items():
  for t in ([1] if arm.startswith('first-') else [1,8]):
